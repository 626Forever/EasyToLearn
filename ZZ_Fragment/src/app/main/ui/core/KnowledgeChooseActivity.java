	...
	private void openRecoder(final String detail) {
		...
		if (dirs.size() == 1) {
			...
			if (filename.equals(getString(R.string.file_media_video))) {
				...
				builder.setNeutralButton("删除录像", new OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
				...
				builder.setNeutralButton("删除录音", new OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						fileModule.deleteFile(dirs.get(0));
						dialog.dismiss();
					}
			...
			builder.setNeutralButton("删除", new OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					AlertDialog.Builder abuilder = new Builder(
							KnowledgeChooseActivity.this);
					abuilder.setMessage("亲，请选择删除放录像还是删除录音");
					abuilder.setTitle("提示");
					abuilder.setPositiveButton("删除录像", new OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
							for (int i = 0; i < dirs.size(); i++) {
								String name = dirs.get(i);
								if (StringUtility.getFileName(name).equals(
					...
					abuilder.setNegativeButton("删除录音", new OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
							for (int i = 0; i < dirs.size(); i++) {
								String name = dirs.get(i);
